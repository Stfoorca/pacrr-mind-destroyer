...
def load_query_idf(qids, doc_mat_dir):
    ...
    for qid in qids:
        ...
        qid_desc_idf[qid] = qid_topic_idf[qid]
    ...
    for qid in qids:
        ...
        for cwid in qid_cwid_rmat[qid]:
            ...
            if qid not in qid_ext_idfarr:
                print('{}'.format(qid_term_idf[qid]))
                print('{}'.format(np.pad(qid_term_idf[qid], pad_width=((0,max_query_term-len_query)),mode='constant', constant_values=-np.inf)))
                print('{}'.format(qid_cwid_rmat[qid][cwid].shape[0]))
                print('{} {} {} {}'.format(qid_term_idf[qid].shape, np.pad(qid_term_idf[qid], pad_width=((0,max_query_term-len_query)),mode='constant', constant_values=-np.inf).shape, max_query_term, len_query))
...
def load_test_data(qids, rawdoc_mat_dir, qid_cwid_label, N_GRAMS, param_val):
    ...
    contexts = []
    for qid in qids:
        print('{} {}'.format(qid, qid_cwid_label))
        print('OTYSUKO---------------------------------------------')
        assert 1==0
