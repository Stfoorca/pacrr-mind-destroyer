...
import argparse
import itertools
from utils.utils2 import read_file, read_qrels, read_query, preprocess_text
...
if __name__ == '__main__':
    ...
    qrel_files = '/home/pepe/Desktop/paccr-master/qrels'
    topic_files = '/home/pepe/Desktop/pacrr/query.xml'
    remove_stopwords = False
    corpus_folder = '/home/pepe/Desktop/paccr-master/relevant'
    outdir = '/home/pepe/Desktop/paccr-master/topic_idf2'
    # Argument handling
    #args = argument_parser(sys.argv[1:])
    
    # Read QRels files
    qrels = []
    qrels += read_qrels(qrel_files)

    # Read topics and unique cwids
    query_id2text = read_query(topic_files)
    ...
    pool = mp.Pool(
        mp.cpu_count(),
        initializer=process,
        initargs=(q_process_recv, q_process_send, corpus_folder, remove_stopwords)
    )
    ...
    topics = [
        preprocess_text(query_id2text[qid]['query'], tokenize=True, all_lower=True, stopw=remove_stopwords).split()
        for qid in sorted(query_id2text.keys())
    ]
    print(topics)


    for queries, f_name in zip([topics], ['topic']):
        ...
        if not os.path.isdir("%s/%s" % (outdir, f_name)):
            os.makedirs("%s/%s" % (outdir, f_name))
        ...
        for query, qid in tqdm(zip(queries, sorted(query_id2text.keys())), total=len(queries), desc='Saving IDF vectors'):
            ...
            # Save IDF array
            np.save("%s/%s/%s.npy" % (outdir, f_name, qid), query_idf)